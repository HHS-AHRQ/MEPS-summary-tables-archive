...
proc sort data = all_events;
 by ind DUPERSID VARSTR VARPSU PERWT06F Condition ind count;
run;
...
data persNA; set all_pers;
 array vars XP06X SF06X MR06X MD06X PR06X OZ06X;
 do over vars;
...
proc surveymeans data = persNA sum ;
 ...
 cluster VARPSU;
 weight PERWT06F;
 var XP06X SF06X MR06X MD06X PR06X OZ06X;
 domain Condition;
run;
