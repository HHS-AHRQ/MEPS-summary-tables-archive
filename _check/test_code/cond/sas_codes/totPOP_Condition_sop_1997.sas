...
proc sort data = all_events;
 by ind DUPERSID VARSTR VARPSU PERWT97F Condition ind count;
run;
...
data persNA; set all_pers;
 array vars XP97X SF97X MR97X MD97X PR97X OZ97X;
 do over vars;
...
proc surveymeans data = persNA sum ;
 ...
 cluster VARPSU;
 weight PERWT97F;
 var XP97X SF97X MR97X MD97X PR97X OZ97X;
 domain Condition;
run;
