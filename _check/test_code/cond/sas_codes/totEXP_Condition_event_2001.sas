...
data MEPS; set MEPS;
 ...
 OMAEXP01 = VISEXP01 + OTHEXP01;

 TOTUSE01 = 
  ((DVTOT01 > 0) + (RXTOT01 > 0) + (OBTOTV01 > 0) +
...
data FYCsub; set MEPS;
 keep ind DUPERSID PERWT01F VARSTR VARPSU;
run;
...
proc sort data = all_events;
 by ind DUPERSID VARSTR VARPSU PERWT01F Condition event ind count;
run;
