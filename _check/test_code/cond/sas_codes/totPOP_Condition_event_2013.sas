...
data MEPS; set MEPS;
 ...
 OMAEXP13 = VISEXP13 + OTHEXP13;

 TOTUSE13 = 
  ((DVTOT13 > 0) + (RXTOT13 > 0) + (OBTOTV13 > 0) +
...
data FYCsub; set MEPS;
 keep ind DUPERSID PERWT13F VARSTR VARPSU;
run;
...
proc sort data = all_events;
 by ind DUPERSID VARSTR VARPSU PERWT13F Condition event ind count;
run;
