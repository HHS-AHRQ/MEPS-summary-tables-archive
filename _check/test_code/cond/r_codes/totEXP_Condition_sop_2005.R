...
# Add aggregate sources of payment
  if(year <= 1999)
    FYC <- FYC %>% mutate(TOTTRI05 = TOTCHM05)

  FYC <- FYC %>% mutate(
    TOTOTH05 = TOTOFD05 + TOTSTL05 + TOTOPR05 + TOTOPU05 + TOTOSR05,
    TOTOTZ05 = TOTOTH05 + TOTWCP05 + TOTVA05,
    TOTPTR05 = TOTPRV05 + TOTTRI05)

# Keep only needed variables from FYC
  FYCsub <- FYC %>% select(ind, DUPERSID, PERWT05F, VARSTR, VARPSU)
...
svyby(~XP05X + SF05X + MR05X + MD05X + PR05X + OZ05X, by = ~Condition, FUN = svytotal, design = EVNTdsgn)
