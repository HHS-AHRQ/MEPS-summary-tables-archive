...
# Keep only needed variables from FYC
  FYCsub <- FYC %>% select(agegrps,ind, DUPERSID, PERWT00F, VARSTR, VARPSU)
...
all_pers <- all_events %>%
  group_by(agegrps,ind, DUPERSID, VARSTR, VARPSU, PERWT00F, Condition, count) %>%
...
svyby(~XP00X, by = ~Condition + agegrps, FUN = svymean, design = PERSdsgn)
