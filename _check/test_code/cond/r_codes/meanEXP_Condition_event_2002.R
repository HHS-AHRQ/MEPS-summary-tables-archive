...
# Add aggregate event variables
  FYC <- FYC %>% mutate(
    HHTEXP02 = HHAEXP02 + HHNEXP02, # Home Health Agency + Independent providers
    ERTEXP02 = ERFEXP02 + ERDEXP02, # Doctor + Facility Expenses for OP, ER, IP events
    IPTEXP02 = IPFEXP02 + IPDEXP02,
    OPTEXP02 = OPFEXP02 + OPDEXP02, # All Outpatient
    OPYEXP02 = OPVEXP02 + OPSEXP02, # Physician only
    OPZEXP02 = OPOEXP02 + OPPEXP02, # Non-physician only
    OMAEXP02 = VISEXP02 + OTHEXP02) # Other medical equipment and services

  FYC <- FYC %>% mutate(
    TOTUSE02 = ((DVTOT02 > 0) + (RXTOT02 > 0) + (OBTOTV02 > 0) +
                  (OPTOTV02 > 0) + (ERTOT02 > 0) + (IPDIS02 > 0) +
                  (HHTOTD02 > 0) + (OMAEXP02 > 0))
  )

# Keep only needed variables from FYC
  FYCsub <- FYC %>% select(ind, DUPERSID, PERWT02F, VARSTR, VARPSU)
...
all_persev <- all_events %>%
  group_by(ind, DUPERSID, VARSTR, VARPSU, PERWT02F, Condition, event, count) %>%
  summarize_at(vars(SF02X, PR02X, MR02X, MD02X, OZ02X, XP02X),sum) %>% ungroup
...
svyby(~XP02X, by = ~Condition + event,FUN = svymean, design = PERSevnt)
