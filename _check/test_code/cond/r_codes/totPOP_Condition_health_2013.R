...
# Keep only needed variables from FYC
  FYCsub <- FYC %>% select(health,ind, DUPERSID, PERWT13F, VARSTR, VARPSU)
...
all_pers <- all_events %>%
  group_by(health,ind, DUPERSID, VARSTR, VARPSU, PERWT13F, Condition, count) %>%
...
svyby(~count, by = ~Condition + health, FUN = svytotal, design = PERSdsgn)
