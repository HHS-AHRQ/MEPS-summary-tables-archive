...
# Poverty status
  ...
  FYC <- FYC %>%
    mutate(poverty = recode_factor(POVCAT13, .default = "Missing",
      "1" = "Negative or poor",
      "2" = "Near-poor",
      "3" = "Low income",
      "4" = "Middle income",
...
# Keep only needed variables from FYC
  FYCsub <- FYC %>% select(poverty,ind, DUPERSID, PERWT13F, VARSTR, VARPSU)
...
all_pers <- all_events %>%
  group_by(poverty,ind, DUPERSID, VARSTR, VARPSU, PERWT13F, Condition, count) %>%
...
svyby(~XP13X, by = ~Condition + poverty, FUN = svymean, design = PERSdsgn)
