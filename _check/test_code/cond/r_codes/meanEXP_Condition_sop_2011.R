...
# Add aggregate sources of payment
  if(year <= 1999)
    FYC <- FYC %>% mutate(TOTTRI11 = TOTCHM11)
...
all_pers <- all_events %>%
  group_by(ind, DUPERSID, VARSTR, VARPSU, PERWT11F, Condition, count) %>%
...
svyby(~XP11X + SF11X + MR11X + MD11X + PR11X + OZ11X, by = ~Condition, FUN = svymean, design = PERSdsgn)
