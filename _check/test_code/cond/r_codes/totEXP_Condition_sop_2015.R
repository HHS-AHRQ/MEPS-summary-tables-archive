...
# Add aggregate sources of payment
  if(year <= 1999)
    FYC <- FYC %>% mutate(TOTTRI15 = TOTCHM15)

  FYC <- FYC %>% mutate(
    TOTOTH15 = TOTOFD15 + TOTSTL15 + TOTOPR15 + TOTOPU15 + TOTOSR15,
    TOTOTZ15 = TOTOTH15 + TOTWCP15 + TOTVA15,
    TOTPTR15 = TOTPRV15 + TOTTRI15)
...
svyby(~XP15X + SF15X + MR15X + MD15X + PR15X + OZ15X, by = ~Condition, FUN = svytotal, design = EVNTdsgn)
