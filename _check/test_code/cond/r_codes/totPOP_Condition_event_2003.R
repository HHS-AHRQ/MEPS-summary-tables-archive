...
# Add aggregate event variables
  FYC <- FYC %>% mutate(
    HHTEXP03 = HHAEXP03 + HHNEXP03, # Home Health Agency + Independent providers
    ERTEXP03 = ERFEXP03 + ERDEXP03, # Doctor + Facility Expenses for OP, ER, IP events
    IPTEXP03 = IPFEXP03 + IPDEXP03,
    OPTEXP03 = OPFEXP03 + OPDEXP03, # All Outpatient
    OPYEXP03 = OPVEXP03 + OPSEXP03, # Physician only
    OPZEXP03 = OPOEXP03 + OPPEXP03, # Non-physician only
    OMAEXP03 = VISEXP03 + OTHEXP03) # Other medical equipment and services

  FYC <- FYC %>% mutate(
    TOTUSE03 = ((DVTOT03 > 0) + (RXTOT03 > 0) + (OBTOTV03 > 0) +
                  (OPTOTV03 > 0) + (ERTOT03 > 0) + (IPDIS03 > 0) +
                  (HHTOTD03 > 0) + (OMAEXP03 > 0))
  )
...
all_persev <- all_events %>%
  group_by(ind, DUPERSID, VARSTR, VARPSU, PERWT03F, Condition, event, count) %>%
