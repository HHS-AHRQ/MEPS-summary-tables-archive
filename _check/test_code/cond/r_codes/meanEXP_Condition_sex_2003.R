...
# Keep only needed variables from FYC
  FYCsub <- FYC %>% select(sex,ind, DUPERSID, PERWT03F, VARSTR, VARPSU)
...
all_pers <- all_events %>%
  group_by(sex,ind, DUPERSID, VARSTR, VARPSU, PERWT03F, Condition, count) %>%
...
svyby(~XP03X, by = ~Condition + sex, FUN = svymean, design = PERSdsgn)
