...
# Add aggregate sources of payment
  if(year <= 1999)
    FYC <- FYC %>% mutate(TOTTRI98 = TOTCHM98)
...
all_pers <- all_events %>%
  group_by(ind, DUPERSID, VARSTR, VARPSU, PERWT98F, Condition, count) %>%
  summarize_at(vars(SF98X, PR98X, MR98X, MD98X, OZ98X, XP98X),sum) %>% ungroup
...
svyby(~XP98X + SF98X + MR98X + MD98X + PR98X + OZ98X, by = ~Condition, FUN = svymean, design = PERSdsgn)
