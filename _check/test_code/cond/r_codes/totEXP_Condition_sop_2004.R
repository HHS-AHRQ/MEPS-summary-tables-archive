...
# Add aggregate sources of payment
  if(year <= 1999)
    FYC <- FYC %>% mutate(TOTTRI04 = TOTCHM04)

  FYC <- FYC %>% mutate(
    TOTOTH04 = TOTOFD04 + TOTSTL04 + TOTOPR04 + TOTOPU04 + TOTOSR04,
    TOTOTZ04 = TOTOTH04 + TOTWCP04 + TOTVA04,
    TOTPTR04 = TOTPRV04 + TOTTRI04)

# Keep only needed variables from FYC
  FYCsub <- FYC %>% select(ind, DUPERSID, PERWT04F, VARSTR, VARPSU)
...
svyby(~XP04X + SF04X + MR04X + MD04X + PR04X + OZ04X, by = ~Condition, FUN = svytotal, design = EVNTdsgn)
