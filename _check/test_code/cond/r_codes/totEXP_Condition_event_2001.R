...
# Add aggregate event variables
  FYC <- FYC %>% mutate(
    HHTEXP01 = HHAEXP01 + HHNEXP01, # Home Health Agency + Independent providers
    ERTEXP01 = ERFEXP01 + ERDEXP01, # Doctor + Facility Expenses for OP, ER, IP events
    IPTEXP01 = IPFEXP01 + IPDEXP01,
    OPTEXP01 = OPFEXP01 + OPDEXP01, # All Outpatient
    OPYEXP01 = OPVEXP01 + OPSEXP01, # Physician only
    OPZEXP01 = OPOEXP01 + OPPEXP01, # Non-physician only
    OMAEXP01 = VISEXP01 + OTHEXP01) # Other medical equipment and services

  FYC <- FYC %>% mutate(
    TOTUSE01 = ((DVTOT01 > 0) + (RXTOT01 > 0) + (OBTOTV01 > 0) +
                  (OPTOTV01 > 0) + (ERTOT01 > 0) + (IPDIS01 > 0) +
                  (HHTOTD01 > 0) + (OMAEXP01 > 0))
  )
...
all_persev <- all_events %>%
  group_by(ind, DUPERSID, VARSTR, VARPSU, PERWT01F, Condition, event, count) %>%
...
svyby(~XP01X, by = ~Condition + event, FUN = svytotal, design = PERSevnt)
