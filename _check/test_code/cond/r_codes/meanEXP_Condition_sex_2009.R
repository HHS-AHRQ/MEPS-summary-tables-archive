...
# Keep only needed variables from FYC
  FYCsub <- FYC %>% select(sex,ind, DUPERSID, PERWT09F, VARSTR, VARPSU)
...
all_pers <- all_events %>%
  group_by(sex,ind, DUPERSID, VARSTR, VARPSU, PERWT09F, Condition, count) %>%
...
svyby(~XP09X, by = ~Condition + sex, FUN = svymean, design = PERSdsgn)
