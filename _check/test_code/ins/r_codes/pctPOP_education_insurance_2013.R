...
# To compute for all insurance categories, replace 'insurance' in the 'svyby' function with 'insurance_v2X'
  ...
  if(year < 2011){
    FYC <- FYC %>%
      mutate(
        public   = (MCDEV13==1|OPAEV13==1|OPBEV13==1),
        medicare = (MCREV13==1),
        private  = (INSCOV13==1),
        ...
        INSURC13 = ifelse(AGELAST < 65, INSCOV13, ins_gt65)
      )
  }

  FYC <- FYC %>%
    mutate(insurance = recode_factor(INSCOV13, .default = "Missing",
      "1" = "Any private, all ages",
      "2" = "Public only, all ages",
      "3" = "Uninsured, all ages")) %>%
    mutate(insurance_v2X = recode_factor(INSURC13, .default = "Missing",
      "1" = "<65, Any private",
      "2" = "<65, Public only",
      "3" = "<65, Uninsured",
      "4" = "65+, Medicare only",
...
# Education
  if(year <= 1998){
    FYC <- FYC %>% mutate(EDUCYR = EDUCYR13)
  }else if(year <= 2004){
