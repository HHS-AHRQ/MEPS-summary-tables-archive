...
# To compute for all insurance categories, replace 'insurance' in the 'svyby' function with 'insurance_v2X'
  ...
  if(year < 2011){
    FYC <- FYC %>%
      mutate(
        public   = (MCDEV06==1|OPAEV06==1|OPBEV06==1),
        medicare = (MCREV06==1),
        private  = (INSCOV06==1),
        ...
        INSURC06 = ifelse(AGELAST < 65, INSCOV06, ins_gt65)
      )
  }

  FYC <- FYC %>%
    mutate(insurance = recode_factor(INSCOV06, .default = "Missing",
      "1" = "Any private, all ages",
      "2" = "Public only, all ages",
      "3" = "Uninsured, all ages")) %>%
    mutate(insurance_v2X = recode_factor(INSURC06, .default = "Missing",
      "1" = "<65, Any private",
      "2" = "<65, Public only",
      "3" = "<65, Uninsured",
      "4" = "65+, Medicare only",
...
# Education
  if(year <= 1998){
    FYC <- FYC %>% mutate(EDUCYR = EDUCYR06)
  }else if(year <= 2004){
...
svyby(~insurance, FUN = svytotal, by = ~education, design = FYCdsgn)
