...
# To compute for all insurance categories, replace 'insurance' in the 'svyby' function with 'insurance_v2X'
  ...
  if(year < 2011){
    FYC <- FYC %>%
      mutate(
        public   = (MCDEV14==1|OPAEV14==1|OPBEV14==1),
        medicare = (MCREV14==1),
        private  = (INSCOV14==1),
        ...
        INSURC14 = ifelse(AGELAST < 65, INSCOV14, ins_gt65)
      )
  }

  FYC <- FYC %>%
    mutate(insurance = recode_factor(INSCOV14, .default = "Missing",
      "1" = "Any private, all ages",
      "2" = "Public only, all ages",
      "3" = "Uninsured, all ages")) %>%
    mutate(insurance_v2X = recode_factor(INSURC14, .default = "Missing",
      "1" = "<65, Any private",
      "2" = "<65, Public only",
      "3" = "<65, Uninsured",
      "4" = "65+, Medicare only",
...
# Poverty status
  ...
  FYC <- FYC %>%
    mutate(poverty = recode_factor(POVCAT14, .default = "Missing",
      "1" = "Negative or poor",
      "2" = "Near-poor",
      "3" = "Low income",
      "4" = "Middle income",
...
svyby(~insurance, FUN = svytotal, by = ~poverty, design = FYCdsgn)
