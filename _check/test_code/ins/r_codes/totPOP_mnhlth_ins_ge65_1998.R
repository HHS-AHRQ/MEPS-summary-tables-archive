...
# To compute for all insurance categories, replace 'insurance' in the 'svyby' function with 'insurance_v2X'
  ...
  if(year < 2011){
    FYC <- FYC %>%
      mutate(
        public   = (MCDEV98==1|OPAEV98==1|OPBEV98==1),
        medicare = (MCREV98==1),
        private  = (INSCOV98==1),
        ...
        INSURC98 = ifelse(AGELAST < 65, INSCOV98, ins_gt65)
      )
  }

  FYC <- FYC %>%
    mutate(insurance = recode_factor(INSCOV98, .default = "Missing",
      "1" = "Any private, all ages",
      "2" = "Public only, all ages",
      "3" = "Uninsured, all ages")) %>%
    mutate(insurance_v2X = recode_factor(INSURC98, .default = "Missing",
      "1" = "<65, Any private",
      "2" = "<65, Public only",
      "3" = "<65, Uninsured",
      "4" = "65+, Medicare only",
...
svyby(~insurance_v2X, FUN = svytotal, by = ~mnhlth, design = subset(FYCdsgn, AGELAST >= 65))
