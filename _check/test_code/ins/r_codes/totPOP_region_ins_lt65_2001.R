...
# To compute for all insurance categories, replace 'insurance' in the 'svyby' function with 'insurance_v2X'
  ...
  if(year < 2011){
    FYC <- FYC %>%
      mutate(
        public   = (MCDEV01==1|OPAEV01==1|OPBEV01==1),
        medicare = (MCREV01==1),
        private  = (INSCOV01==1),
        ...
        INSURC01 = ifelse(AGELAST < 65, INSCOV01, ins_gt65)
      )
  }

  FYC <- FYC %>%
    mutate(insurance = recode_factor(INSCOV01, .default = "Missing",
      "1" = "Any private, all ages",
      "2" = "Public only, all ages",
      "3" = "Uninsured, all ages")) %>%
    mutate(insurance_v2X = recode_factor(INSURC01, .default = "Missing",
      "1" = "<65, Any private",
      "2" = "<65, Public only",
      "3" = "<65, Uninsured",
      "4" = "65+, Medicare only",
...
# Census region
  ...
  FYC <- FYC %>%
    ...
    mutate(region = coalesce(REGION01, REGION42, REGION31)) %>%
...
svyby(~insurance_v2X, FUN = svytotal, by = ~region, design = subset(FYCdsgn, AGELAST < 65))
