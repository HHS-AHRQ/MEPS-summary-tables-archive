...
# To compute for all insurance categories, replace 'insurance' in the 'svyby' function with 'insurance_v2X'
  ...
  if(year < 2011){
    FYC <- FYC %>%
      mutate(
        public   = (MCDEV03==1|OPAEV03==1|OPBEV03==1),
        medicare = (MCREV03==1),
        private  = (INSCOV03==1),
        ...
        INSURC03 = ifelse(AGELAST < 65, INSCOV03, ins_gt65)
      )
  }

  FYC <- FYC %>%
    mutate(insurance = recode_factor(INSCOV03, .default = "Missing",
      "1" = "Any private, all ages",
      "2" = "Public only, all ages",
      "3" = "Uninsured, all ages")) %>%
    mutate(insurance_v2X = recode_factor(INSURC03, .default = "Missing",
      "1" = "<65, Any private",
      "2" = "<65, Public only",
      "3" = "<65, Uninsured",
      "4" = "65+, Medicare only",
...
svyby(~insurance_v2X, FUN = svymean, by = ~poverty, design = subset(FYCdsgn, AGELAST >= 65))
