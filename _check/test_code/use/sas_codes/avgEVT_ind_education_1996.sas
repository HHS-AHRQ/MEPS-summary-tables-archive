...
/* Education */
data MEPS; set MEPS;
 ARRAY EDUVARS(4) EDUCYR96 EDUCYR EDUCYEAR EDRECODE;
 if year <= 1998 then EDUCYR = EDUCYR96;
...
data FYCsub; set MEPS;
 keep ind education DUPERSID PERWT96F VARSTR VARPSU;
run;
...
proc means data = pers_events sum noprint;
 by DUPERSID VARSTR VARPSU PERWT96F ind education ;
 var EXP;
