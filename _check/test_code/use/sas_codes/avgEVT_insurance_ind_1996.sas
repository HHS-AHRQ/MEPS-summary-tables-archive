...
data FYCsub; set MEPS;
 keep insurance ind DUPERSID PERWT96F VARSTR VARPSU;
run;
...
proc means data = pers_events sum noprint;
 by DUPERSID VARSTR VARPSU PERWT96F insurance ind ;
 var EXP;
...
proc surveymeans data = n_events mean missing nobs;
 FORMAT insurance insurance. ind ind.;
 VAR EXP;
 STRATA VARSTR;
 CLUSTER VARPSU;
 WEIGHT PERWT96F;
 DOMAIN insurance*ind;
run;
