...
/* Education */
data MEPS; set MEPS;
 ARRAY EDUVARS(4) EDUCYR15 EDUCYR EDUCYEAR EDRECODE;
 if year <= 1998 then EDUCYR = EDUCYR15;
...
data FYCsub; set MEPS;
 keep education ind DUPERSID PERWT15F VARSTR VARPSU;
run;
...
proc means data = pers_events sum noprint;
 by DUPERSID VARSTR VARPSU PERWT15F education ind ;
 var EXP;
