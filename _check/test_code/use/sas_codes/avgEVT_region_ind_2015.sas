...
data MEPS; set MEPS;
 ...
 if REGION15 >= 0 then region = REGION15;
...
data FYCsub; set MEPS;
 keep region ind DUPERSID PERWT15F VARSTR VARPSU;
run;
...
proc means data = pers_events sum noprint;
 by DUPERSID VARSTR VARPSU PERWT15F region ind ;
 var EXP;
