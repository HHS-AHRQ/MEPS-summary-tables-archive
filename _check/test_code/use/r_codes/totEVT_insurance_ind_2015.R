...
# To compute for all insurance categories, replace 'insurance' in the 'svyby' function with 'insurance_v2X'
  ...
  if(year < 2011){
    FYC <- FYC %>%
      mutate(
        public   = (MCDEV15==1|OPAEV15==1|OPBEV15==1),
        medicare = (MCREV15==1),
        private  = (INSCOV15==1),
        ...
        INSURC15 = ifelse(AGELAST < 65, INSCOV15, ins_gt65)
      )
  }

  FYC <- FYC %>%
    mutate(insurance = recode_factor(INSCOV15, .default = "Missing",
      "1" = "Any private, all ages",
      "2" = "Public only, all ages",
      "3" = "Uninsured, all ages")) %>%
    mutate(insurance_v2X = recode_factor(INSURC15, .default = "Missing",
      "1" = "<65, Any private",
      "2" = "<65, Public only",
      "3" = "<65, Uninsured",
      "4" = "65+, Medicare only",
...
# Keep only needed variables from FYC
  FYCsub <- FYC %>% select(insurance,ind, DUPERSID, PERWT15F, VARSTR, VARPSU)
...
svyby(~(XP15X >= 0), FUN=svytotal, by = ~insurance + ind, design = subset(EVNTdsgn, XP15X >= 0))
