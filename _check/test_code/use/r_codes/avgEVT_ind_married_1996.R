...
# Marital status
  ...
  FYC <- FYC %>%
    ...
    mutate(married = coalesce(MARRY96X, MARRY42X, MARRY31X)) %>%
...
# Keep only needed variables from FYC
  FYCsub <- FYC %>% select(ind,married, DUPERSID, PERWT96F, VARSTR, VARPSU)
...
svyby(~ANY, FUN=svymean, by = ~ind + married, design = nEVTdsgn)
