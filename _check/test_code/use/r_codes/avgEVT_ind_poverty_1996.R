...
# Poverty status
  ...
  FYC <- FYC %>%
    mutate(poverty = recode_factor(POVCAT96, .default = "Missing",
      "1" = "Negative or poor",
      "2" = "Near-poor",
      "3" = "Low income",
      "4" = "Middle income",
...
# Keep only needed variables from FYC
  FYCsub <- FYC %>% select(ind,poverty, DUPERSID, PERWT96F, VARSTR, VARPSU)
...
svyby(~ANY, FUN=svymean, by = ~ind + poverty, design = nEVTdsgn)
