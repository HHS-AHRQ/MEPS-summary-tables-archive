...
# Keep only needed variables from FYC
  FYCsub <- FYC %>% select(ind,education, DUPERSID, PERWT13F, VARSTR, VARPSU)
...
# Stack events
  ...
  stacked_events <- stacked_events %>%
    ...
    select(DUPERSID, event, event_v2X, SEEDOC,
      XP13X, SF13X, MR13X, MD13X, PR13X, OZ13X)
...
svyby(~XP13X, FUN=svymean, by = ~ind + education, design = subset(EVNTdsgn, XP13X >= 0))
