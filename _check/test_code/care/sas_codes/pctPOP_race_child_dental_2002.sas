...
proc surveyfreq data = MEPS missing;
 FORMAT child_dental child_dental. race race.;
 STRATA VARSTR;
 CLUSTER VARPSU;
 WEIGHT PERWT02F;
 TABLES domain*race*child_dental / row;
run;

proc print data = out;
 where domain = 1 and child_dental ne . and race ne .;
 var child_dental race WgtFreq StdDev Frequency RowPercent RowStdErr;
run;
