...
proc surveyfreq data = MEPS missing;
 FORMAT child_listen freq. education education.;
 STRATA VARSTR;
 CLUSTER VARPSU;
 WEIGHT PERWT08F;
 TABLES domain*education*child_listen / row;
run;

proc print data = out;
 where domain = 1 and child_listen ne . and education ne .;
 var child_listen education WgtFreq StdDev Frequency RowPercent RowStdErr;
run;
