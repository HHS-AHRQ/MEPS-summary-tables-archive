...
proc surveyfreq data = MEPS missing;
 FORMAT adult_listen freq. education education.;
 STRATA VARSTR;
 CLUSTER VARPSU;
 WEIGHT SAQWT12F;
 TABLES domain*education*adult_listen / row;
run;

proc print data = out;
 where domain = 1 and adult_listen ne . and education ne .;
 var adult_listen education WgtFreq StdDev Frequency RowPercent RowStdErr;
run;
