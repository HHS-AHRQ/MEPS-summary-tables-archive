...
data MEPS; set MEPS;
 ...
 if domain = 0 and SAQWT04F = 0 then SAQWT04F = 1;
run;
...
proc surveyfreq data = MEPS missing;
 FORMAT adult_routine freq. race race.;
 STRATA VARSTR;
 CLUSTER VARPSU;
 WEIGHT SAQWT04F;
 TABLES domain*race*adult_routine / row;
run;

proc print data = out;
 where domain = 1 and adult_routine ne . and race ne .;
 var adult_routine race WgtFreq StdDev Frequency RowPercent RowStdErr;
run;
