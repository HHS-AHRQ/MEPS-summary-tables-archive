...
# To compute for all insurance categories, replace 'insurance' in the 'svyby' function with 'insurance_v2X'
  ...
  if(year < 2011){
    FYC <- FYC %>%
      mutate(
        public   = (MCDEV04==1|OPAEV04==1|OPBEV04==1),
        medicare = (MCREV04==1),
        private  = (INSCOV04==1),
        ...
        INSURC04 = ifelse(AGELAST < 65, INSCOV04, ins_gt65)
      )
  }

  FYC <- FYC %>%
    mutate(insurance = recode_factor(INSCOV04, .default = "Missing",
      "1" = "Any private, all ages",
      "2" = "Public only, all ages",
      "3" = "Uninsured, all ages")) %>%
    mutate(insurance_v2X = recode_factor(INSURC04, .default = "Missing",
      "1" = "<65, Any private",
      "2" = "<65, Public only",
      "3" = "<65, Uninsured",
      "4" = "65+, Medicare only",
...
svyby(~afford_DN + insure_DN + other_DN, FUN = svytotal, by = ~insurance, design = subset(FYCdsgn, ACCELI42==1 & delay_DN==1))
