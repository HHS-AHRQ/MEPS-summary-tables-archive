...
# To compute for all insurance categories, replace 'insurance' in the 'svyby' function with 'insurance_v2X'
  ...
  if(year < 2011){
    FYC <- FYC %>%
      mutate(
        public   = (MCDEV08==1|OPAEV08==1|OPBEV08==1),
        medicare = (MCREV08==1),
        private  = (INSCOV08==1),
        ...
        INSURC08 = ifelse(AGELAST < 65, INSCOV08, ins_gt65)
      )
  }

  FYC <- FYC %>%
    mutate(insurance = recode_factor(INSCOV08, .default = "Missing",
      "1" = "Any private, all ages",
      "2" = "Public only, all ages",
      "3" = "Uninsured, all ages")) %>%
    mutate(insurance_v2X = recode_factor(INSURC08, .default = "Missing",
      "1" = "<65, Any private",
      "2" = "<65, Public only",
      "3" = "<65, Uninsured",
      "4" = "65+, Medicare only",
...
svyby(~child_explain, FUN=svymean, by = ~insurance, design = subset(FYCdsgn, CHAPPT42 >= 1 & AGELAST < 18))
