...
# Keep only needed variables from FYC
  FYCsub <- FYC %>% select(poverty,ind, DUPERSID, PERWT.yy.F, VARSTR, VARPSU)
...
# Sum by person, condition, across event
  all_pers <- all_events %>%
    group_by(poverty,ind, DUPERSID, VARSTR, VARPSU, PERWT.yy.F, Condition, count) %>%
...
results <- svyby(~XP.yy.X, by = ~Condition + poverty, FUN = svymean, design = PERSdsgn)
print(results)
