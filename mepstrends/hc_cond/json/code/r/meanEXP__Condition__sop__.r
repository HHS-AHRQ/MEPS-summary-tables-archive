...
# Sum by person, condition, across event
  all_pers <- all_events %>%
    group_by(ind, DUPERSID, VARSTR, VARPSU, PERWT.yy.F, Condition, count) %>%
...
results <- svyby(~XP.yy.X + SF.yy.X + MR.yy.X + MD.yy.X + PR.yy.X + OZ.yy.X, by = ~Condition, FUN = svymean, design = PERSdsgn)
print(results)
