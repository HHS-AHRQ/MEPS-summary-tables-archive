...
proc surveyfreq data = MEPS missing;
  FORMAT adult_listen freq. region region.;
  STRATA VARSTR;
  CLUSTER VARPSU;
  WEIGHT SAQWT&yy.F;
  TABLES domain*region*adult_listen / row;
run;

proc print data = out;
  where domain = 1 and adult_listen ne . and region ne .;
  var adult_listen region WgtFreq StdDev Frequency RowPercent RowStdErr;
run;
