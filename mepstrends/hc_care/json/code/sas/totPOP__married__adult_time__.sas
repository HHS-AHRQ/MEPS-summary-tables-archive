...
proc surveyfreq data = MEPS missing;
  FORMAT adult_time freq. married married.;
  STRATA VARSTR;
  CLUSTER VARPSU;
  WEIGHT SAQWT&yy.F;
  TABLES domain*married*adult_time / row;
run;

proc print data = out;
  where domain = 1 and adult_time ne . and married ne .;
  var adult_time married WgtFreq StdDev Frequency RowPercent RowStdErr;
run;
