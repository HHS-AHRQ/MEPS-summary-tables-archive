...
proc surveyfreq data = MEPS missing;
  FORMAT adult_respect freq. agegrps agegrps.;
  STRATA VARSTR;
  CLUSTER VARPSU;
  WEIGHT SAQWT&yy.F;
  TABLES domain*agegrps*adult_respect / row;
run;

proc print data = out;
  where domain = 1 and adult_respect ne . and agegrps ne .;
  var adult_respect agegrps WgtFreq StdDev Frequency RowPercent RowStdErr;
run;
