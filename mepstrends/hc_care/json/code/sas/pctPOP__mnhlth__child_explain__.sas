...
proc surveyfreq data = MEPS missing;
  FORMAT child_explain freq. mnhlth mnhlth.;
  STRATA VARSTR;
  CLUSTER VARPSU;
  WEIGHT PERWT&yy.F;
  TABLES domain*mnhlth*child_explain / row;
run;

proc print data = out;
  where domain = 1 and child_explain ne . and mnhlth ne .;
  var child_explain mnhlth WgtFreq StdDev Frequency RowPercent RowStdErr;
run;
