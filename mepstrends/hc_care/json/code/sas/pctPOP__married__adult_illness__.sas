...
proc surveyfreq data = MEPS missing;
  FORMAT adult_illness freq. married married.;
  STRATA VARSTR;
  CLUSTER VARPSU;
  WEIGHT SAQWT&yy.F;
  TABLES domain*married*adult_illness / row;
run;

proc print data = out;
  where domain = 1 and adult_illness ne . and married ne .;
  var adult_illness married WgtFreq StdDev Frequency RowPercent RowStdErr;
run;
