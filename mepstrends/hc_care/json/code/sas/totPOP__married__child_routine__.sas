...
proc surveyfreq data = MEPS missing;
  FORMAT child_routine freq. married married.;
  STRATA VARSTR;
  CLUSTER VARPSU;
  WEIGHT PERWT&yy.F;
  TABLES domain*married*child_routine / row;
run;

proc print data = out;
  where domain = 1 and child_routine ne . and married ne .;
  var child_routine married WgtFreq StdDev Frequency RowPercent RowStdErr;
run;
