...
proc surveyfreq data = MEPS missing;
  FORMAT adult_time freq. insurance insurance.;
  STRATA VARSTR;
  CLUSTER VARPSU;
  WEIGHT SAQWT&yy.F;
  TABLES domain*insurance*adult_time / row;
run;

proc print data = out;
  where domain = 1 and adult_time ne . and insurance ne .;
  var adult_time insurance WgtFreq StdDev Frequency RowPercent RowStdErr;
run;
