...
proc surveyfreq data = MEPS missing;
  FORMAT adult_respect freq. region region.;
  STRATA VARSTR;
  CLUSTER VARPSU;
  WEIGHT SAQWT&yy.F;
  TABLES domain*region*adult_respect / row;
run;

proc print data = out;
  where domain = 1 and adult_respect ne . and region ne .;
  var adult_respect region WgtFreq StdDev Frequency RowPercent RowStdErr;
run;
