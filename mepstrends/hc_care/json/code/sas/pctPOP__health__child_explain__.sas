...
proc surveyfreq data = MEPS missing;
  FORMAT child_explain freq. health health.;
  STRATA VARSTR;
  CLUSTER VARPSU;
  WEIGHT PERWT&yy.F;
  TABLES domain*health*child_explain / row;
run;

proc print data = out;
  where domain = 1 and child_explain ne . and health ne .;
  var child_explain health WgtFreq StdDev Frequency RowPercent RowStdErr;
run;
