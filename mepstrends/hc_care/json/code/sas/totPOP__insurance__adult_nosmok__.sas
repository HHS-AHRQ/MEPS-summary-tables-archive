...
proc surveyfreq data = MEPS missing;
  FORMAT adult_nosmok adult_nosmok. insurance insurance.;
  STRATA VARSTR;
  CLUSTER VARPSU;
  WEIGHT SAQWT&yy.F;
  TABLES domain*insurance*adult_nosmok / row;
run;

proc print data = out;
  where domain = 1 and adult_nosmok ne . and insurance ne .;
  var adult_nosmok insurance WgtFreq StdDev Frequency RowPercent RowStdErr;
run;
