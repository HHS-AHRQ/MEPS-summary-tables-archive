...
proc surveyfreq data = MEPS missing;
  FORMAT child_illness freq. insurance insurance.;
  STRATA VARSTR;
  CLUSTER VARPSU;
  WEIGHT PERWT&yy.F;
  TABLES domain*insurance*child_illness / row;
run;

proc print data = out;
  where domain = 1 and child_illness ne . and insurance ne .;
  var child_illness insurance WgtFreq StdDev Frequency RowPercent RowStdErr;
run;
