...
proc surveyfreq data = MEPS missing;
  FORMAT diab_foot diab_foot. ind ind.;
  STRATA VARSTR;
  CLUSTER VARPSU;
  WEIGHT DIABW&yy.F;
  TABLES domain*ind*diab_foot / row;
run;

proc print data = out;
  where domain = 1 and diab_foot ne . and ind ne .;
  var diab_foot ind WgtFreq StdDev Frequency RowPercent RowStdErr;
run;
