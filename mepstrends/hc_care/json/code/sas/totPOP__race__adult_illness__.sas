...
proc surveyfreq data = MEPS missing;
  FORMAT adult_illness freq. race race.;
  STRATA VARSTR;
  CLUSTER VARPSU;
  WEIGHT SAQWT&yy.F;
  TABLES domain*race*adult_illness / row;
run;

proc print data = out;
  where domain = 1 and adult_illness ne . and race ne .;
  var adult_illness race WgtFreq StdDev Frequency RowPercent RowStdErr;
run;
