...
proc surveyfreq data = MEPS missing;
  FORMAT child_time freq. married married.;
  STRATA VARSTR;
  CLUSTER VARPSU;
  WEIGHT PERWT&yy.F;
  TABLES domain*married*child_time / row;
run;

proc print data = out;
  where domain = 1 and child_time ne . and married ne .;
  var child_time married WgtFreq StdDev Frequency RowPercent RowStdErr;
run;
