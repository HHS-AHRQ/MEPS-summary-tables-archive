...
proc surveyfreq data = MEPS missing;
  FORMAT child_respect freq. insurance insurance.;
  STRATA VARSTR;
  CLUSTER VARPSU;
  WEIGHT PERWT&yy.F;
  TABLES domain*insurance*child_respect / row;
run;

proc print data = out;
  where domain = 1 and child_respect ne . and insurance ne .;
  var child_respect insurance WgtFreq StdDev Frequency RowPercent RowStdErr;
run;
