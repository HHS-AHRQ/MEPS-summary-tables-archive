...
proc surveyfreq data = MEPS missing;
  FORMAT adult_explain freq. married married.;
  STRATA VARSTR;
  CLUSTER VARPSU;
  WEIGHT SAQWT&yy.F;
  TABLES domain*married*adult_explain / row;
run;

proc print data = out;
  where domain = 1 and adult_explain ne . and married ne .;
  var adult_explain married WgtFreq StdDev Frequency RowPercent RowStdErr;
run;
