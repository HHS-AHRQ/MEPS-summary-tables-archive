...
proc surveyfreq data = MEPS missing;
  FORMAT diab_chol diab_chol. race race.;
  STRATA VARSTR;
  CLUSTER VARPSU;
  WEIGHT DIABW&yy.F;
  TABLES domain*race*diab_chol / row;
run;

proc print data = out;
  where domain = 1 and diab_chol ne . and race ne .;
  var diab_chol race WgtFreq StdDev Frequency RowPercent RowStdErr;
run;
