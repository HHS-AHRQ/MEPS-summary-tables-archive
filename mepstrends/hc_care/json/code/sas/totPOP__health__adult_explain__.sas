...
proc surveyfreq data = MEPS missing;
  FORMAT adult_explain freq. health health.;
  STRATA VARSTR;
  CLUSTER VARPSU;
  WEIGHT SAQWT&yy.F;
  TABLES domain*health*adult_explain / row;
run;

proc print data = out;
  where domain = 1 and adult_explain ne . and health ne .;
  var adult_explain health WgtFreq StdDev Frequency RowPercent RowStdErr;
run;
