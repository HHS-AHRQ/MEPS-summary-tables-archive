...
proc surveyfreq data = MEPS missing;
  FORMAT adult_respect freq. mnhlth mnhlth.;
  STRATA VARSTR;
  CLUSTER VARPSU;
  WEIGHT SAQWT&yy.F;
  TABLES domain*mnhlth*adult_respect / row;
run;

proc print data = out;
  where domain = 1 and adult_respect ne . and mnhlth ne .;
  var adult_respect mnhlth WgtFreq StdDev Frequency RowPercent RowStdErr;
run;
