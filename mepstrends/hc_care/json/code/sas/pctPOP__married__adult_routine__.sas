...
proc surveyfreq data = MEPS missing;
  FORMAT adult_routine freq. married married.;
  STRATA VARSTR;
  CLUSTER VARPSU;
  WEIGHT SAQWT&yy.F;
  TABLES domain*married*adult_routine / row;
run;

proc print data = out;
  where domain = 1 and adult_routine ne . and married ne .;
  var adult_routine married WgtFreq StdDev Frequency RowPercent RowStdErr;
run;
