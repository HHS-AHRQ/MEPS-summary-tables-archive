...
proc surveyfreq data = MEPS missing;
  FORMAT diab_chol diab_chol. mnhlth mnhlth.;
  STRATA VARSTR;
  CLUSTER VARPSU;
  WEIGHT DIABW&yy.F;
  TABLES domain*mnhlth*diab_chol / row;
run;

proc print data = out;
  where domain = 1 and diab_chol ne . and mnhlth ne .;
  var diab_chol mnhlth WgtFreq StdDev Frequency RowPercent RowStdErr;
run;
