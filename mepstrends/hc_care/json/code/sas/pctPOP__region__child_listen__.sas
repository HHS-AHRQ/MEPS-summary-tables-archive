...
proc surveyfreq data = MEPS missing;
  FORMAT child_listen freq. region region.;
  STRATA VARSTR;
  CLUSTER VARPSU;
  WEIGHT PERWT&yy.F;
  TABLES domain*region*child_listen / row;
run;

proc print data = out;
  where domain = 1 and child_listen ne . and region ne .;
  var child_listen region WgtFreq StdDev Frequency RowPercent RowStdErr;
run;
