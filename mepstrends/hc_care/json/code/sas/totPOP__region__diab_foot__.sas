...
proc surveyfreq data = MEPS missing;
  FORMAT diab_foot diab_foot. region region.;
  STRATA VARSTR;
  CLUSTER VARPSU;
  WEIGHT DIABW&yy.F;
  TABLES domain*region*diab_foot / row;
run;

proc print data = out;
  where domain = 1 and diab_foot ne . and region ne .;
  var diab_foot region WgtFreq StdDev Frequency RowPercent RowStdErr;
run;
