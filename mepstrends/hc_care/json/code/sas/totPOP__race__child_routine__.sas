...
proc surveyfreq data = MEPS missing;
  FORMAT child_routine freq. race race.;
  STRATA VARSTR;
  CLUSTER VARPSU;
  WEIGHT PERWT&yy.F;
  TABLES domain*race*child_routine / row;
run;

proc print data = out;
  where domain = 1 and child_routine ne . and race ne .;
  var child_routine race WgtFreq StdDev Frequency RowPercent RowStdErr;
run;
