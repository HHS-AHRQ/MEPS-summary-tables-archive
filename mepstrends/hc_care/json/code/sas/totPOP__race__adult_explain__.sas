...
proc surveyfreq data = MEPS missing;
  FORMAT adult_explain freq. race race.;
  STRATA VARSTR;
  CLUSTER VARPSU;
  WEIGHT SAQWT&yy.F;
  TABLES domain*race*adult_explain / row;
run;

proc print data = out;
  where domain = 1 and adult_explain ne . and race ne .;
  var adult_explain race WgtFreq StdDev Frequency RowPercent RowStdErr;
run;
