...
proc surveyfreq data = MEPS missing;
  FORMAT child_explain freq. education education.;
  STRATA VARSTR;
  CLUSTER VARPSU;
  WEIGHT PERWT&yy.F;
  TABLES domain*education*child_explain / row;
run;

proc print data = out;
  where domain = 1 and child_explain ne . and education ne .;
  var child_explain education WgtFreq StdDev Frequency RowPercent RowStdErr;
run;
