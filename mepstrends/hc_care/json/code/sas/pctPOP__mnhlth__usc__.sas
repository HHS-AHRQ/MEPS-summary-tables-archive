...
proc surveyfreq data = MEPS missing;
  FORMAT usc usc. mnhlth mnhlth.;
  STRATA VARSTR;
  CLUSTER VARPSU;
  WEIGHT PERWT&yy.F;
  TABLES domain*mnhlth*usc / row;
run;

proc print data = out;
  where domain = 1 and usc ne . and mnhlth ne .;
  var usc mnhlth WgtFreq StdDev Frequency RowPercent RowStdErr;
run;
