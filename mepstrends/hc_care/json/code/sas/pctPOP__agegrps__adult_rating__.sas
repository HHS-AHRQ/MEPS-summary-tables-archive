...
proc surveyfreq data = MEPS missing;
  FORMAT adult_rating adult_rating. agegrps agegrps.;
  STRATA VARSTR;
  CLUSTER VARPSU;
  WEIGHT SAQWT&yy.F;
  TABLES domain*agegrps*adult_rating / row;
run;

proc print data = out;
  where domain = 1 and adult_rating ne . and agegrps ne .;
  var adult_rating agegrps WgtFreq StdDev Frequency RowPercent RowStdErr;
run;
