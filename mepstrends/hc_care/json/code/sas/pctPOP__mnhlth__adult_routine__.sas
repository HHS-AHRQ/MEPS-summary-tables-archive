...
proc surveyfreq data = MEPS missing;
  FORMAT adult_routine freq. mnhlth mnhlth.;
  STRATA VARSTR;
  CLUSTER VARPSU;
  WEIGHT SAQWT&yy.F;
  TABLES domain*mnhlth*adult_routine / row;
run;

proc print data = out;
  where domain = 1 and adult_routine ne . and mnhlth ne .;
  var adult_routine mnhlth WgtFreq StdDev Frequency RowPercent RowStdErr;
run;
