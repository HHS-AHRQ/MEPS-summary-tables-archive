...
proc surveyfreq data = MEPS missing;
  FORMAT child_respect freq. race race.;
  STRATA VARSTR;
  CLUSTER VARPSU;
  WEIGHT PERWT&yy.F;
  TABLES domain*race*child_respect / row;
run;

proc print data = out;
  where domain = 1 and child_respect ne . and race ne .;
  var child_respect race WgtFreq StdDev Frequency RowPercent RowStdErr;
run;
