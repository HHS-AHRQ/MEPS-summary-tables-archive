...
proc surveyfreq data = MEPS missing;
  FORMAT child_time freq. race race.;
  STRATA VARSTR;
  CLUSTER VARPSU;
  WEIGHT PERWT&yy.F;
  TABLES domain*race*child_time / row;
run;

proc print data = out;
  where domain = 1 and child_time ne . and race ne .;
  var child_time race WgtFreq StdDev Frequency RowPercent RowStdErr;
run;
