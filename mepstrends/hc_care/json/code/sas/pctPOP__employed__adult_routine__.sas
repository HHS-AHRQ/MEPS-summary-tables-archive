...
proc surveyfreq data = MEPS missing;
  FORMAT adult_routine freq. employed employed.;
  STRATA VARSTR;
  CLUSTER VARPSU;
  WEIGHT SAQWT&yy.F;
  TABLES domain*employed*adult_routine / row;
run;

proc print data = out;
  where domain = 1 and adult_routine ne . and employed ne .;
  var adult_routine employed WgtFreq StdDev Frequency RowPercent RowStdErr;
run;
