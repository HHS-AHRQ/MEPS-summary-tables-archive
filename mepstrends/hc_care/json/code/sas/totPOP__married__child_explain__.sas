...
proc surveyfreq data = MEPS missing;
  FORMAT child_explain freq. married married.;
  STRATA VARSTR;
  CLUSTER VARPSU;
  WEIGHT PERWT&yy.F;
  TABLES domain*married*child_explain / row;
run;

proc print data = out;
  where domain = 1 and child_explain ne . and married ne .;
  var child_explain married WgtFreq StdDev Frequency RowPercent RowStdErr;
run;
