...
proc surveyfreq data = MEPS missing;
  FORMAT child_rating child_rating. race race.;
  STRATA VARSTR;
  CLUSTER VARPSU;
  WEIGHT PERWT&yy.F;
  TABLES domain*race*child_rating / row;
run;

proc print data = out;
  where domain = 1 and child_rating ne . and race ne .;
  var child_rating race WgtFreq StdDev Frequency RowPercent RowStdErr;
run;
