...
proc surveyfreq data = MEPS missing;
  FORMAT afford_PM afford. insure_PM insure. other_PM other. race race.;
  STRATA VARSTR;
  CLUSTER VARPSU;
  WEIGHT PERWT&yy.F;
  TABLES domain*race*(afford_PM insure_PM other_PM) / row;
run;

proc print data = out;
  where domain = 1 and (afford_PM > 0 or insure_PM > 0 or other_PM > 0) and race ne .;
  var afford_PM insure_PM other_PM race WgtFreq StdDev Frequency RowPercent RowStdErr;
run;
