...
proc surveyfreq data = MEPS missing;
  FORMAT adult_time freq. agegrps agegrps.;
  STRATA VARSTR;
  CLUSTER VARPSU;
  WEIGHT SAQWT&yy.F;
  TABLES domain*agegrps*adult_time / row;
run;

proc print data = out;
  where domain = 1 and adult_time ne . and agegrps ne .;
  var adult_time agegrps WgtFreq StdDev Frequency RowPercent RowStdErr;
run;
