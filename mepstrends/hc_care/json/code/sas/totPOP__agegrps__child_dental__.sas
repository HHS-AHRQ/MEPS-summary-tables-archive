...
proc surveyfreq data = MEPS missing;
  FORMAT child_dental child_dental. agegrps agegrps.;
  STRATA VARSTR;
  CLUSTER VARPSU;
  WEIGHT PERWT&yy.F;
  TABLES domain*agegrps*child_dental / row;
run;

proc print data = out;
  where domain = 1 and child_dental ne . and agegrps ne .;
  var child_dental agegrps WgtFreq StdDev Frequency RowPercent RowStdErr;
run;
