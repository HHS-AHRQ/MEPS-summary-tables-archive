...
proc surveyfreq data = MEPS missing;
  FORMAT diab_eye diab_eye. agegrps agegrps.;
  STRATA VARSTR;
  CLUSTER VARPSU;
  WEIGHT DIABW&yy.F;
  TABLES domain*agegrps*diab_eye / row;
run;

proc print data = out;
  where domain = 1 and diab_eye ne . and agegrps ne .;
  var diab_eye agegrps WgtFreq StdDev Frequency RowPercent RowStdErr;
run;
