...
proc surveyfreq data = MEPS missing;
  FORMAT child_time freq. region region.;
  STRATA VARSTR;
  CLUSTER VARPSU;
  WEIGHT PERWT&yy.F;
  TABLES domain*region*child_time / row;
run;

proc print data = out;
  where domain = 1 and child_time ne . and region ne .;
  var child_time region WgtFreq StdDev Frequency RowPercent RowStdErr;
run;
