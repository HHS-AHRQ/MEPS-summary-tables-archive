...
proc surveyfreq data = MEPS missing;
  FORMAT adult_listen freq. race race.;
  STRATA VARSTR;
  CLUSTER VARPSU;
  WEIGHT SAQWT&yy.F;
  TABLES domain*race*adult_listen / row;
run;

proc print data = out;
  where domain = 1 and adult_listen ne . and race ne .;
  var adult_listen race WgtFreq StdDev Frequency RowPercent RowStdErr;
run;
