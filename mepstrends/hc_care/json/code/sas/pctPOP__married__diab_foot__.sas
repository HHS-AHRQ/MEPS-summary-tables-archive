...
proc surveyfreq data = MEPS missing;
  FORMAT diab_foot diab_foot. married married.;
  STRATA VARSTR;
  CLUSTER VARPSU;
  WEIGHT DIABW&yy.F;
  TABLES domain*married*diab_foot / row;
run;

proc print data = out;
  where domain = 1 and diab_foot ne . and married ne .;
  var diab_foot married WgtFreq StdDev Frequency RowPercent RowStdErr;
run;
