...
proc surveyfreq data = MEPS missing;
  FORMAT child_routine freq. insurance insurance.;
  STRATA VARSTR;
  CLUSTER VARPSU;
  WEIGHT PERWT&yy.F;
  TABLES domain*insurance*child_routine / row;
run;

proc print data = out;
  where domain = 1 and child_routine ne . and insurance ne .;
  var child_routine insurance WgtFreq StdDev Frequency RowPercent RowStdErr;
run;
