...
proc surveyfreq data = MEPS missing;
  FORMAT adult_illness freq. poverty poverty.;
  STRATA VARSTR;
  CLUSTER VARPSU;
  WEIGHT SAQWT&yy.F;
  TABLES domain*poverty*adult_illness / row;
run;

proc print data = out;
  where domain = 1 and adult_illness ne . and poverty ne .;
  var adult_illness poverty WgtFreq StdDev Frequency RowPercent RowStdErr;
run;
