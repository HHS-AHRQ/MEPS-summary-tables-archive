...
proc surveyfreq data = MEPS missing;
  FORMAT adult_listen freq. insurance insurance.;
  STRATA VARSTR;
  CLUSTER VARPSU;
  WEIGHT SAQWT&yy.F;
  TABLES domain*insurance*adult_listen / row;
run;

proc print data = out;
  where domain = 1 and adult_listen ne . and insurance ne .;
  var adult_listen insurance WgtFreq StdDev Frequency RowPercent RowStdErr;
run;
