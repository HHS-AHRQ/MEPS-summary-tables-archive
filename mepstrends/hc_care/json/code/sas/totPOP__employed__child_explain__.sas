...
proc surveyfreq data = MEPS missing;
  FORMAT child_explain freq. employed employed.;
  STRATA VARSTR;
  CLUSTER VARPSU;
  WEIGHT PERWT&yy.F;
  TABLES domain*employed*child_explain / row;
run;

proc print data = out;
  where domain = 1 and child_explain ne . and employed ne .;
  var child_explain employed WgtFreq StdDev Frequency RowPercent RowStdErr;
run;
