...
proc surveyfreq data = MEPS missing;
  FORMAT child_routine freq. education education.;
  STRATA VARSTR;
  CLUSTER VARPSU;
  WEIGHT PERWT&yy.F;
  TABLES domain*education*child_routine / row;
run;

proc print data = out;
  where domain = 1 and child_routine ne . and education ne .;
  var child_routine education WgtFreq StdDev Frequency RowPercent RowStdErr;
run;
