...
proc surveyfreq data = MEPS missing;
  FORMAT diab_eye diab_eye. employed employed.;
  STRATA VARSTR;
  CLUSTER VARPSU;
  WEIGHT DIABW&yy.F;
  TABLES domain*employed*diab_eye / row;
run;

proc print data = out;
  where domain = 1 and diab_eye ne . and employed ne .;
  var diab_eye employed WgtFreq StdDev Frequency RowPercent RowStdErr;
run;
