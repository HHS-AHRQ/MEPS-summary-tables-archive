...
proc surveyfreq data = MEPS missing;
  FORMAT child_explain freq. region region.;
  STRATA VARSTR;
  CLUSTER VARPSU;
  WEIGHT PERWT&yy.F;
  TABLES domain*region*child_explain / row;
run;

proc print data = out;
  where domain = 1 and child_explain ne . and region ne .;
  var child_explain region WgtFreq StdDev Frequency RowPercent RowStdErr;
run;
