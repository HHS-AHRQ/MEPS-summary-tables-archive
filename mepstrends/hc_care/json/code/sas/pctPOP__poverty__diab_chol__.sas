...
proc surveyfreq data = MEPS missing;
  FORMAT diab_chol diab_chol. poverty poverty.;
  STRATA VARSTR;
  CLUSTER VARPSU;
  WEIGHT DIABW&yy.F;
  TABLES domain*poverty*diab_chol / row;
run;

proc print data = out;
  where domain = 1 and diab_chol ne . and poverty ne .;
  var diab_chol poverty WgtFreq StdDev Frequency RowPercent RowStdErr;
run;
