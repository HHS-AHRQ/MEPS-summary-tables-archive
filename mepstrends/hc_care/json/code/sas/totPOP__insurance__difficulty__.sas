...
proc surveyfreq data = MEPS missing;
  FORMAT delay: delay. insurance insurance.;
  STRATA VARSTR;
  CLUSTER VARPSU;
  WEIGHT PERWT&yy.F;
  TABLES domain*insurance*(delay_ANY delay_MD delay_DN delay_PM) / row;
run;

proc print data = out;
  where domain = 1 and (delay_ANY > 0 or delay_MD > 0 or delay_DN > 0 or delay_PM > 0) and insurance ne .;
  var delay_ANY delay_MD delay_DN delay_PM insurance WgtFreq StdDev Frequency RowPercent RowStdErr;
run;
