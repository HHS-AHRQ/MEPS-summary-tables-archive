...
proc surveyfreq data = MEPS missing;
  FORMAT child_time freq. health health.;
  STRATA VARSTR;
  CLUSTER VARPSU;
  WEIGHT PERWT&yy.F;
  TABLES domain*health*child_time / row;
run;

proc print data = out;
  where domain = 1 and child_time ne . and health ne .;
  var child_time health WgtFreq StdDev Frequency RowPercent RowStdErr;
run;
