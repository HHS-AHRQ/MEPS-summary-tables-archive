...
proc surveyfreq data = MEPS missing;
  FORMAT adult_respect freq. insurance insurance.;
  STRATA VARSTR;
  CLUSTER VARPSU;
  WEIGHT SAQWT&yy.F;
  TABLES domain*insurance*adult_respect / row;
run;

proc print data = out;
  where domain = 1 and adult_respect ne . and insurance ne .;
  var adult_respect insurance WgtFreq StdDev Frequency RowPercent RowStdErr;
run;
