...
proc surveyfreq data = MEPS missing;
  FORMAT adult_time freq. mnhlth mnhlth.;
  STRATA VARSTR;
  CLUSTER VARPSU;
  WEIGHT SAQWT&yy.F;
  TABLES domain*mnhlth*adult_time / row;
run;

proc print data = out;
  where domain = 1 and adult_time ne . and mnhlth ne .;
  var adult_time mnhlth WgtFreq StdDev Frequency RowPercent RowStdErr;
run;
