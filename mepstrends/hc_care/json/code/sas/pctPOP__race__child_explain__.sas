...
proc surveyfreq data = MEPS missing;
  FORMAT child_explain freq. race race.;
  STRATA VARSTR;
  CLUSTER VARPSU;
  WEIGHT PERWT&yy.F;
  TABLES domain*race*child_explain / row;
run;

proc print data = out;
  where domain = 1 and child_explain ne . and race ne .;
  var child_explain race WgtFreq StdDev Frequency RowPercent RowStdErr;
run;
