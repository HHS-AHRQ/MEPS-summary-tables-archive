...
proc surveyfreq data = MEPS missing;
  FORMAT adult_illness freq. region region.;
  STRATA VARSTR;
  CLUSTER VARPSU;
  WEIGHT SAQWT&yy.F;
  TABLES domain*region*adult_illness / row;
run;

proc print data = out;
  where domain = 1 and adult_illness ne . and region ne .;
  var adult_illness region WgtFreq StdDev Frequency RowPercent RowStdErr;
run;
