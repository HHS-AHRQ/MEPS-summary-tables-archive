...
proc surveyfreq data = MEPS missing;
  FORMAT child_respect freq. mnhlth mnhlth.;
  STRATA VARSTR;
  CLUSTER VARPSU;
  WEIGHT PERWT&yy.F;
  TABLES domain*mnhlth*child_respect / row;
run;

proc print data = out;
  where domain = 1 and child_respect ne . and mnhlth ne .;
  var child_respect mnhlth WgtFreq StdDev Frequency RowPercent RowStdErr;
run;
