...
proc surveyfreq data = MEPS missing;
  FORMAT adult_respect freq. race race.;
  STRATA VARSTR;
  CLUSTER VARPSU;
  WEIGHT SAQWT&yy.F;
  TABLES domain*race*adult_respect / row;
run;

proc print data = out;
  where domain = 1 and adult_respect ne . and race ne .;
  var adult_respect race WgtFreq StdDev Frequency RowPercent RowStdErr;
run;
