...
proc surveyfreq data = MEPS missing;
  FORMAT child_routine freq. region region.;
  STRATA VARSTR;
  CLUSTER VARPSU;
  WEIGHT PERWT&yy.F;
  TABLES domain*region*child_routine / row;
run;

proc print data = out;
  where domain = 1 and child_routine ne . and region ne .;
  var child_routine region WgtFreq StdDev Frequency RowPercent RowStdErr;
run;
