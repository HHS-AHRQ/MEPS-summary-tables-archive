...
proc surveyfreq data = MEPS missing;
  FORMAT child_routine freq. health health.;
  STRATA VARSTR;
  CLUSTER VARPSU;
  WEIGHT PERWT&yy.F;
  TABLES domain*health*child_routine / row;
run;

proc print data = out;
  where domain = 1 and child_routine ne . and health ne .;
  var child_routine health WgtFreq StdDev Frequency RowPercent RowStdErr;
run;
