...
proc surveyfreq data = MEPS missing;
  FORMAT child_listen freq. employed employed.;
  STRATA VARSTR;
  CLUSTER VARPSU;
  WEIGHT PERWT&yy.F;
  TABLES domain*employed*child_listen / row;
run;

proc print data = out;
  where domain = 1 and child_listen ne . and employed ne .;
  var child_listen employed WgtFreq StdDev Frequency RowPercent RowStdErr;
run;
