...
proc surveyfreq data = MEPS missing;
  FORMAT diab_chol diab_chol. ind ind.;
  STRATA VARSTR;
  CLUSTER VARPSU;
  WEIGHT DIABW&yy.F;
  TABLES domain*ind*diab_chol / row;
run;

proc print data = out;
  where domain = 1 and diab_chol ne . and ind ne .;
  var diab_chol ind WgtFreq StdDev Frequency RowPercent RowStdErr;
run;
