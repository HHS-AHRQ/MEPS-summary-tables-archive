...
proc surveyfreq data = MEPS missing;
  FORMAT adult_listen freq. agegrps agegrps.;
  STRATA VARSTR;
  CLUSTER VARPSU;
  WEIGHT SAQWT&yy.F;
  TABLES domain*agegrps*adult_listen / row;
run;

proc print data = out;
  where domain = 1 and adult_listen ne . and agegrps ne .;
  var adult_listen agegrps WgtFreq StdDev Frequency RowPercent RowStdErr;
run;
