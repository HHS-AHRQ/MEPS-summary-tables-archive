...
proc surveyfreq data = MEPS missing;
  FORMAT diab_chol diab_chol. insurance insurance.;
  STRATA VARSTR;
  CLUSTER VARPSU;
  WEIGHT DIABW&yy.F;
  TABLES domain*insurance*diab_chol / row;
run;

proc print data = out;
  where domain = 1 and diab_chol ne . and insurance ne .;
  var diab_chol insurance WgtFreq StdDev Frequency RowPercent RowStdErr;
run;
