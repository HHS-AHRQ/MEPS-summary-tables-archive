...
proc surveyfreq data = MEPS missing;
  FORMAT child_respect freq. employed employed.;
  STRATA VARSTR;
  CLUSTER VARPSU;
  WEIGHT PERWT&yy.F;
  TABLES domain*employed*child_respect / row;
run;

proc print data = out;
  where domain = 1 and child_respect ne . and employed ne .;
  var child_respect employed WgtFreq StdDev Frequency RowPercent RowStdErr;
run;
