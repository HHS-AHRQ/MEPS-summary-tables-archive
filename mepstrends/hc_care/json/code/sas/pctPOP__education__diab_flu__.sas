...
proc surveyfreq data = MEPS missing;
  FORMAT diab_flu diab_flu. education education.;
  STRATA VARSTR;
  CLUSTER VARPSU;
  WEIGHT DIABW&yy.F;
  TABLES domain*education*diab_flu / row;
run;

proc print data = out;
  where domain = 1 and diab_flu ne . and education ne .;
  var diab_flu education WgtFreq StdDev Frequency RowPercent RowStdErr;
run;
