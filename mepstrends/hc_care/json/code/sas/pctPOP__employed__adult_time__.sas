...
proc surveyfreq data = MEPS missing;
  FORMAT adult_time freq. employed employed.;
  STRATA VARSTR;
  CLUSTER VARPSU;
  WEIGHT SAQWT&yy.F;
  TABLES domain*employed*adult_time / row;
run;

proc print data = out;
  where domain = 1 and adult_time ne . and employed ne .;
  var adult_time employed WgtFreq StdDev Frequency RowPercent RowStdErr;
run;
