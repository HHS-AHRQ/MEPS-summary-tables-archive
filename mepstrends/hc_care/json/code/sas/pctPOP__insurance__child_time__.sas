...
proc surveyfreq data = MEPS missing;
  FORMAT child_time freq. insurance insurance.;
  STRATA VARSTR;
  CLUSTER VARPSU;
  WEIGHT PERWT&yy.F;
  TABLES domain*insurance*child_time / row;
run;

proc print data = out;
  where domain = 1 and child_time ne . and insurance ne .;
  var child_time insurance WgtFreq StdDev Frequency RowPercent RowStdErr;
run;
