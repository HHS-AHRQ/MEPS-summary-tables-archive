...
proc surveyfreq data = MEPS missing;
  FORMAT adult_routine freq. education education.;
  STRATA VARSTR;
  CLUSTER VARPSU;
  WEIGHT SAQWT&yy.F;
  TABLES domain*education*adult_routine / row;
run;

proc print data = out;
  where domain = 1 and adult_routine ne . and education ne .;
  var adult_routine education WgtFreq StdDev Frequency RowPercent RowStdErr;
run;
