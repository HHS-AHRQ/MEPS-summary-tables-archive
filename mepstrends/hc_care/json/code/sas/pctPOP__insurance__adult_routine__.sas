...
proc surveyfreq data = MEPS missing;
  FORMAT adult_routine freq. insurance insurance.;
  STRATA VARSTR;
  CLUSTER VARPSU;
  WEIGHT SAQWT&yy.F;
  TABLES domain*insurance*adult_routine / row;
run;

proc print data = out;
  where domain = 1 and adult_routine ne . and insurance ne .;
  var adult_routine insurance WgtFreq StdDev Frequency RowPercent RowStdErr;
run;
