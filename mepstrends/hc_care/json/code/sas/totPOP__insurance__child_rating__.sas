...
proc surveyfreq data = MEPS missing;
  FORMAT child_rating child_rating. insurance insurance.;
  STRATA VARSTR;
  CLUSTER VARPSU;
  WEIGHT PERWT&yy.F;
  TABLES domain*insurance*child_rating / row;
run;

proc print data = out;
  where domain = 1 and child_rating ne . and insurance ne .;
  var child_rating insurance WgtFreq StdDev Frequency RowPercent RowStdErr;
run;
