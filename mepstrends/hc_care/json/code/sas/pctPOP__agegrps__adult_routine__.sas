...
proc surveyfreq data = MEPS missing;
  FORMAT adult_routine freq. agegrps agegrps.;
  STRATA VARSTR;
  CLUSTER VARPSU;
  WEIGHT SAQWT&yy.F;
  TABLES domain*agegrps*adult_routine / row;
run;

proc print data = out;
  where domain = 1 and adult_routine ne . and agegrps ne .;
  var adult_routine agegrps WgtFreq StdDev Frequency RowPercent RowStdErr;
run;
