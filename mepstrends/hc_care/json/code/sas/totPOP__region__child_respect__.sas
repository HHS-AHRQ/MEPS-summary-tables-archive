...
proc surveyfreq data = MEPS missing;
  FORMAT child_respect freq. region region.;
  STRATA VARSTR;
  CLUSTER VARPSU;
  WEIGHT PERWT&yy.F;
  TABLES domain*region*child_respect / row;
run;

proc print data = out;
  where domain = 1 and child_respect ne . and region ne .;
  var child_respect region WgtFreq StdDev Frequency RowPercent RowStdErr;
run;
