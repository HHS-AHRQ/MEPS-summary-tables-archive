...
proc surveyfreq data = MEPS missing;
  FORMAT child_routine freq. mnhlth mnhlth.;
  STRATA VARSTR;
  CLUSTER VARPSU;
  WEIGHT PERWT&yy.F;
  TABLES domain*mnhlth*child_routine / row;
run;

proc print data = out;
  where domain = 1 and child_routine ne . and mnhlth ne .;
  var child_routine mnhlth WgtFreq StdDev Frequency RowPercent RowStdErr;
run;
