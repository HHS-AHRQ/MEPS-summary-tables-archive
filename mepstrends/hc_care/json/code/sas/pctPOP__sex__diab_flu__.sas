...
proc surveyfreq data = MEPS missing;
  FORMAT diab_flu diab_flu. sex sex.;
  STRATA VARSTR;
  CLUSTER VARPSU;
  WEIGHT DIABW&yy.F;
  TABLES domain*sex*diab_flu / row;
run;

proc print data = out;
  where domain = 1 and diab_flu ne . and sex ne .;
  var diab_flu sex WgtFreq StdDev Frequency RowPercent RowStdErr;
run;
