...
proc surveyfreq data = MEPS missing;
  FORMAT child_illness freq. health health.;
  STRATA VARSTR;
  CLUSTER VARPSU;
  WEIGHT PERWT&yy.F;
  TABLES domain*health*child_illness / row;
run;

proc print data = out;
  where domain = 1 and child_illness ne . and health ne .;
  var child_illness health WgtFreq StdDev Frequency RowPercent RowStdErr;
run;
