...
proc surveyfreq data = MEPS missing;
  FORMAT adult_respect freq. health health.;
  STRATA VARSTR;
  CLUSTER VARPSU;
  WEIGHT SAQWT&yy.F;
  TABLES domain*health*adult_respect / row;
run;

proc print data = out;
  where domain = 1 and adult_respect ne . and health ne .;
  var adult_respect health WgtFreq StdDev Frequency RowPercent RowStdErr;
run;
