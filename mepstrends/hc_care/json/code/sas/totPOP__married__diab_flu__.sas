...
proc surveyfreq data = MEPS missing;
  FORMAT diab_flu diab_flu. married married.;
  STRATA VARSTR;
  CLUSTER VARPSU;
  WEIGHT DIABW&yy.F;
  TABLES domain*married*diab_flu / row;
run;

proc print data = out;
  where domain = 1 and diab_flu ne . and married ne .;
  var diab_flu married WgtFreq StdDev Frequency RowPercent RowStdErr;
run;
