...
proc surveyfreq data = MEPS missing;
  FORMAT diab_flu diab_flu. poverty poverty.;
  STRATA VARSTR;
  CLUSTER VARPSU;
  WEIGHT DIABW&yy.F;
  TABLES domain*poverty*diab_flu / row;
run;

proc print data = out;
  where domain = 1 and diab_flu ne . and poverty ne .;
  var diab_flu poverty WgtFreq StdDev Frequency RowPercent RowStdErr;
run;
