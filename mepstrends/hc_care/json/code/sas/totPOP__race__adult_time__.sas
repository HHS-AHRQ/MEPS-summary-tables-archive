...
proc surveyfreq data = MEPS missing;
  FORMAT adult_time freq. race race.;
  STRATA VARSTR;
  CLUSTER VARPSU;
  WEIGHT SAQWT&yy.F;
  TABLES domain*race*adult_time / row;
run;

proc print data = out;
  where domain = 1 and adult_time ne . and race ne .;
  var adult_time race WgtFreq StdDev Frequency RowPercent RowStdErr;
run;
