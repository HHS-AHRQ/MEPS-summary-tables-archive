...
proc surveyfreq data = MEPS missing;
  FORMAT adult_respect freq. married married.;
  STRATA VARSTR;
  CLUSTER VARPSU;
  WEIGHT SAQWT&yy.F;
  TABLES domain*married*adult_respect / row;
run;

proc print data = out;
  where domain = 1 and adult_respect ne . and married ne .;
  var adult_respect married WgtFreq StdDev Frequency RowPercent RowStdErr;
run;
