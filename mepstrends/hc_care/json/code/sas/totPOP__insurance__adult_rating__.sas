...
proc surveyfreq data = MEPS missing;
  FORMAT adult_rating adult_rating. insurance insurance.;
  STRATA VARSTR;
  CLUSTER VARPSU;
  WEIGHT SAQWT&yy.F;
  TABLES domain*insurance*adult_rating / row;
run;

proc print data = out;
  where domain = 1 and adult_rating ne . and insurance ne .;
  var adult_rating insurance WgtFreq StdDev Frequency RowPercent RowStdErr;
run;
