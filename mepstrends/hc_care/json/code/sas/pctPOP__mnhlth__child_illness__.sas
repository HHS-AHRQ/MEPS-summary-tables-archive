...
proc surveyfreq data = MEPS missing;
  FORMAT child_illness freq. mnhlth mnhlth.;
  STRATA VARSTR;
  CLUSTER VARPSU;
  WEIGHT PERWT&yy.F;
  TABLES domain*mnhlth*child_illness / row;
run;

proc print data = out;
  where domain = 1 and child_illness ne . and mnhlth ne .;
  var child_illness mnhlth WgtFreq StdDev Frequency RowPercent RowStdErr;
run;
