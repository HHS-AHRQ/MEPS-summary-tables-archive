...
proc surveyfreq data = MEPS missing;
  FORMAT child_respect freq. married married.;
  STRATA VARSTR;
  CLUSTER VARPSU;
  WEIGHT PERWT&yy.F;
  TABLES domain*married*child_respect / row;
run;

proc print data = out;
  where domain = 1 and child_respect ne . and married ne .;
  var child_respect married WgtFreq StdDev Frequency RowPercent RowStdErr;
run;
