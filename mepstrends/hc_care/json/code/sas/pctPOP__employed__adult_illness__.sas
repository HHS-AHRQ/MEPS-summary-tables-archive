...
proc surveyfreq data = MEPS missing;
  FORMAT adult_illness freq. employed employed.;
  STRATA VARSTR;
  CLUSTER VARPSU;
  WEIGHT SAQWT&yy.F;
  TABLES domain*employed*adult_illness / row;
run;

proc print data = out;
  where domain = 1 and adult_illness ne . and employed ne .;
  var adult_illness employed WgtFreq StdDev Frequency RowPercent RowStdErr;
run;
