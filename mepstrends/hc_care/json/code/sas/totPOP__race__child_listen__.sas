...
proc surveyfreq data = MEPS missing;
  FORMAT child_listen freq. race race.;
  STRATA VARSTR;
  CLUSTER VARPSU;
  WEIGHT PERWT&yy.F;
  TABLES domain*race*child_listen / row;
run;

proc print data = out;
  where domain = 1 and child_listen ne . and race ne .;
  var child_listen race WgtFreq StdDev Frequency RowPercent RowStdErr;
run;
