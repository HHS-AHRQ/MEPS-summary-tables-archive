...
proc surveyfreq data = MEPS missing;
  FORMAT child_time freq. education education.;
  STRATA VARSTR;
  CLUSTER VARPSU;
  WEIGHT PERWT&yy.F;
  TABLES domain*education*child_time / row;
run;

proc print data = out;
  where domain = 1 and child_time ne . and education ne .;
  var child_time education WgtFreq StdDev Frequency RowPercent RowStdErr;
run;
