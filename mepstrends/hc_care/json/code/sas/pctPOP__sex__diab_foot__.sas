...
proc surveyfreq data = MEPS missing;
  FORMAT diab_foot diab_foot. sex sex.;
  STRATA VARSTR;
  CLUSTER VARPSU;
  WEIGHT DIABW&yy.F;
  TABLES domain*sex*diab_foot / row;
run;

proc print data = out;
  where domain = 1 and diab_foot ne . and sex ne .;
  var diab_foot sex WgtFreq StdDev Frequency RowPercent RowStdErr;
run;
