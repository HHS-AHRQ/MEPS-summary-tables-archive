...
proc surveyfreq data = MEPS missing;
  FORMAT child_time freq. employed employed.;
  STRATA VARSTR;
  CLUSTER VARPSU;
  WEIGHT PERWT&yy.F;
  TABLES domain*employed*child_time / row;
run;

proc print data = out;
  where domain = 1 and child_time ne . and employed ne .;
  var child_time employed WgtFreq StdDev Frequency RowPercent RowStdErr;
run;
