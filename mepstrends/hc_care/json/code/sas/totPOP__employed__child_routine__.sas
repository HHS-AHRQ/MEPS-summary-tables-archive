...
proc surveyfreq data = MEPS missing;
  FORMAT child_routine freq. employed employed.;
  STRATA VARSTR;
  CLUSTER VARPSU;
  WEIGHT PERWT&yy.F;
  TABLES domain*employed*child_routine / row;
run;

proc print data = out;
  where domain = 1 and child_routine ne . and employed ne .;
  var child_routine employed WgtFreq StdDev Frequency RowPercent RowStdErr;
run;
