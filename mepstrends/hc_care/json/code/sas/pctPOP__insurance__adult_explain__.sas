...
proc surveyfreq data = MEPS missing;
  FORMAT adult_explain freq. insurance insurance.;
  STRATA VARSTR;
  CLUSTER VARPSU;
  WEIGHT SAQWT&yy.F;
  TABLES domain*insurance*adult_explain / row;
run;

proc print data = out;
  where domain = 1 and adult_explain ne . and insurance ne .;
  var adult_explain insurance WgtFreq StdDev Frequency RowPercent RowStdErr;
run;
