...
proc surveyfreq data = MEPS missing;
  FORMAT child_listen freq. insurance insurance.;
  STRATA VARSTR;
  CLUSTER VARPSU;
  WEIGHT PERWT&yy.F;
  TABLES domain*insurance*child_listen / row;
run;

proc print data = out;
  where domain = 1 and child_listen ne . and insurance ne .;
  var child_listen insurance WgtFreq StdDev Frequency RowPercent RowStdErr;
run;
