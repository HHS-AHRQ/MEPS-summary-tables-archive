...
proc surveyfreq data = MEPS missing;
  FORMAT child_rating child_rating. health health.;
  STRATA VARSTR;
  CLUSTER VARPSU;
  WEIGHT PERWT&yy.F;
  TABLES domain*health*child_rating / row;
run;

proc print data = out;
  where domain = 1 and child_rating ne . and health ne .;
  var child_rating health WgtFreq StdDev Frequency RowPercent RowStdErr;
run;
