...
proc surveyfreq data = MEPS missing;
  FORMAT adult_illness freq. education education.;
  STRATA VARSTR;
  CLUSTER VARPSU;
  WEIGHT SAQWT&yy.F;
  TABLES domain*education*adult_illness / row;
run;

proc print data = out;
  where domain = 1 and adult_illness ne . and education ne .;
  var adult_illness education WgtFreq StdDev Frequency RowPercent RowStdErr;
run;
