...
/* Merge with FYC file */
  data FYCsub; set MEPS;
    keep health employed DUPERSID PERWT&yy.F VARSTR VARPSU;
  run;
...
proc means data = pers_events sum noprint;
  by DUPERSID VARSTR VARPSU PERWT&yy.F health employed ;
  var EXP;
...
proc surveymeans data = n_events mean missing nobs;
  FORMAT health health. employed employed.;
  VAR EXP;
  STRATA VARSTR;
  CLUSTER VARPSU;
  WEIGHT PERWT&yy.F;
  DOMAIN health*employed;
run;
