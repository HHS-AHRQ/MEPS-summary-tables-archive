...
/* Merge with FYC file */
  data FYCsub; set MEPS;
    keep region health DUPERSID PERWT&yy.F VARSTR VARPSU;
  run;
...
proc means data = pers_events sum noprint;
  by DUPERSID VARSTR VARPSU PERWT&yy.F region health ;
  var EXP;
...
proc surveymeans data = n_events mean missing nobs;
  FORMAT region region. health health.;
  VAR EXP;
  STRATA VARSTR;
  CLUSTER VARPSU;
  WEIGHT PERWT&yy.F;
  DOMAIN region*health;
run;
