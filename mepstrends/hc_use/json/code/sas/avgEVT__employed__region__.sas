...
/* Merge with FYC file */
  data FYCsub; set MEPS;
    keep employed region DUPERSID PERWT&yy.F VARSTR VARPSU;
  run;
...
proc means data = pers_events sum noprint;
  by DUPERSID VARSTR VARPSU PERWT&yy.F employed region ;
  var EXP;
...
proc surveymeans data = n_events mean missing nobs;
  FORMAT employed employed. region region.;
  VAR EXP;
  STRATA VARSTR;
  CLUSTER VARPSU;
  WEIGHT PERWT&yy.F;
  DOMAIN employed*region;
run;
