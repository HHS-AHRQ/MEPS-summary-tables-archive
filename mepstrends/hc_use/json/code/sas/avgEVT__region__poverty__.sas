...
/* Merge with FYC file */
  data FYCsub; set MEPS;
    keep region poverty DUPERSID PERWT&yy.F VARSTR VARPSU;
  run;
...
proc means data = pers_events sum noprint;
  by DUPERSID VARSTR VARPSU PERWT&yy.F region poverty ;
  var EXP;
...
proc surveymeans data = n_events mean missing nobs;
  FORMAT region region. poverty poverty.;
  VAR EXP;
  STRATA VARSTR;
  CLUSTER VARPSU;
  WEIGHT PERWT&yy.F;
  DOMAIN region*poverty;
run;
