...
/* Merge with FYC file */
  data FYCsub; set MEPS;
    keep employed poverty DUPERSID PERWT&yy.F VARSTR VARPSU;
  run;
...
proc means data = pers_events sum noprint;
  by DUPERSID VARSTR VARPSU PERWT&yy.F employed poverty ;
  var EXP;
...
proc surveymeans data = n_events mean missing nobs;
  FORMAT employed employed. poverty poverty.;
  VAR EXP;
  STRATA VARSTR;
  CLUSTER VARPSU;
  WEIGHT PERWT&yy.F;
  DOMAIN employed*poverty;
run;
