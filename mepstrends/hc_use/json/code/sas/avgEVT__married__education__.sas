...
/* Merge with FYC file */
  data FYCsub; set MEPS;
    keep married education DUPERSID PERWT&yy.F VARSTR VARPSU;
  run;
...
proc means data = pers_events sum noprint;
  by DUPERSID VARSTR VARPSU PERWT&yy.F married education ;
  var EXP;
...
proc surveymeans data = n_events mean missing nobs;
  FORMAT married married. education education.;
  VAR EXP;
  STRATA VARSTR;
  CLUSTER VARPSU;
  WEIGHT PERWT&yy.F;
  DOMAIN married*education;
run;
