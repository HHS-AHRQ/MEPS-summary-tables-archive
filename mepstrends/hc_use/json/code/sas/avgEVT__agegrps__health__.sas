...
/* Merge with FYC file */
  data FYCsub; set MEPS;
    keep agegrps health DUPERSID PERWT&yy.F VARSTR VARPSU;
  run;
...
proc means data = pers_events sum noprint;
  by DUPERSID VARSTR VARPSU PERWT&yy.F agegrps health ;
  var EXP;
...
proc surveymeans data = n_events mean missing nobs;
  FORMAT agegrps agegrps. health health.;
  VAR EXP;
  STRATA VARSTR;
  CLUSTER VARPSU;
  WEIGHT PERWT&yy.F;
  DOMAIN agegrps*health;
run;
