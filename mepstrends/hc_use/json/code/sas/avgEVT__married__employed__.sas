...
/* Merge with FYC file */
  data FYCsub; set MEPS;
    keep married employed DUPERSID PERWT&yy.F VARSTR VARPSU;
  run;
...
proc means data = pers_events sum noprint;
  by DUPERSID VARSTR VARPSU PERWT&yy.F married employed ;
  var EXP;
...
proc surveymeans data = n_events mean missing nobs;
  FORMAT married married. employed employed.;
  VAR EXP;
  STRATA VARSTR;
  CLUSTER VARPSU;
  WEIGHT PERWT&yy.F;
  DOMAIN married*employed;
run;
