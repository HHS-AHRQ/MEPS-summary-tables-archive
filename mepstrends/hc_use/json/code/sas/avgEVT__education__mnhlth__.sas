...
/* Merge with FYC file */
  data FYCsub; set MEPS;
    keep education mnhlth DUPERSID PERWT&yy.F VARSTR VARPSU;
  run;
...
proc means data = pers_events sum noprint;
  by DUPERSID VARSTR VARPSU PERWT&yy.F education mnhlth ;
  var EXP;
...
proc surveymeans data = n_events mean missing nobs;
  FORMAT education education. mnhlth mnhlth.;
  VAR EXP;
  STRATA VARSTR;
  CLUSTER VARPSU;
  WEIGHT PERWT&yy.F;
  DOMAIN education*mnhlth;
run;
