...
/* Merge with FYC file */
  data FYCsub; set MEPS;
    keep insurance region DUPERSID PERWT&yy.F VARSTR VARPSU;
  run;
...
proc means data = pers_events sum noprint;
  by DUPERSID VARSTR VARPSU PERWT&yy.F insurance region ;
  var EXP;
...
proc surveymeans data = n_events mean missing nobs;
  FORMAT insurance insurance. region region.;
  VAR EXP;
  STRATA VARSTR;
  CLUSTER VARPSU;
  WEIGHT PERWT&yy.F;
  DOMAIN insurance*region;
run;
