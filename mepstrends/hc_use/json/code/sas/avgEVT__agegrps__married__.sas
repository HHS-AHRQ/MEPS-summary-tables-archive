...
/* Merge with FYC file */
  data FYCsub; set MEPS;
    keep agegrps married DUPERSID PERWT&yy.F VARSTR VARPSU;
  run;
...
proc means data = pers_events sum noprint;
  by DUPERSID VARSTR VARPSU PERWT&yy.F agegrps married ;
  var EXP;
...
proc surveymeans data = n_events mean missing nobs;
  FORMAT agegrps agegrps. married married.;
  VAR EXP;
  STRATA VARSTR;
  CLUSTER VARPSU;
  WEIGHT PERWT&yy.F;
  DOMAIN agegrps*married;
run;
