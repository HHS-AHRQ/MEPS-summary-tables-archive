...
/* Merge with FYC file */
  data FYCsub; set MEPS;
    keep poverty health DUPERSID PERWT&yy.F VARSTR VARPSU;
  run;
...
proc means data = pers_events sum noprint;
  by DUPERSID VARSTR VARPSU PERWT&yy.F poverty health ;
  var EXP;
...
proc surveymeans data = n_events mean missing nobs;
  FORMAT poverty poverty. health health.;
  VAR EXP;
  STRATA VARSTR;
  CLUSTER VARPSU;
  WEIGHT PERWT&yy.F;
  DOMAIN poverty*health;
run;
