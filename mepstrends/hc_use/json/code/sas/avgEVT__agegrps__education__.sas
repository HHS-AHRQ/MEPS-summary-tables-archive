...
/* Merge with FYC file */
  data FYCsub; set MEPS;
    keep agegrps education DUPERSID PERWT&yy.F VARSTR VARPSU;
  run;
...
proc means data = pers_events sum noprint;
  by DUPERSID VARSTR VARPSU PERWT&yy.F agegrps education ;
  var EXP;
...
proc surveymeans data = n_events mean missing nobs;
  FORMAT agegrps agegrps. education education.;
  VAR EXP;
  STRATA VARSTR;
  CLUSTER VARPSU;
  WEIGHT PERWT&yy.F;
  DOMAIN agegrps*education;
run;
