...
/* Merge with FYC file */
  data FYCsub; set MEPS;
    keep education race DUPERSID PERWT&yy.F VARSTR VARPSU;
  run;
...
proc means data = pers_events sum noprint;
  by DUPERSID VARSTR VARPSU PERWT&yy.F education race ;
  var EXP;
...
proc surveymeans data = n_events mean missing nobs;
  FORMAT education education. race race.;
  VAR EXP;
  STRATA VARSTR;
  CLUSTER VARPSU;
  WEIGHT PERWT&yy.F;
  DOMAIN education*race;
run;
