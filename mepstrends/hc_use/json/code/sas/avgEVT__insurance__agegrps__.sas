...
/* Merge with FYC file */
  data FYCsub; set MEPS;
    keep insurance agegrps DUPERSID PERWT&yy.F VARSTR VARPSU;
  run;
...
proc means data = pers_events sum noprint;
  by DUPERSID VARSTR VARPSU PERWT&yy.F insurance agegrps ;
  var EXP;
...
proc surveymeans data = n_events mean missing nobs;
  FORMAT insurance insurance. agegrps agegrps.;
  VAR EXP;
  STRATA VARSTR;
  CLUSTER VARPSU;
  WEIGHT PERWT&yy.F;
  DOMAIN insurance*agegrps;
run;
