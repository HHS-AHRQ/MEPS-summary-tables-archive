...
/* Merge with FYC file */
  data FYCsub; set MEPS;
    keep poverty employed DUPERSID PERWT&yy.F VARSTR VARPSU;
  run;
...
proc means data = pers_events sum noprint;
  by DUPERSID VARSTR VARPSU PERWT&yy.F poverty employed ;
  var EXP;
...
proc surveymeans data = n_events mean missing nobs;
  FORMAT poverty poverty. employed employed.;
  VAR EXP;
  STRATA VARSTR;
  CLUSTER VARPSU;
  WEIGHT PERWT&yy.F;
  DOMAIN poverty*employed;
run;
