...
/* Merge with FYC file */
  data FYCsub; set MEPS;
    keep education employed DUPERSID PERWT&yy.F VARSTR VARPSU;
  run;
...
proc means data = pers_events sum noprint;
  by DUPERSID VARSTR VARPSU PERWT&yy.F education employed ;
  var EXP;
...
proc surveymeans data = n_events mean missing nobs;
  FORMAT education education. employed employed.;
  VAR EXP;
  STRATA VARSTR;
  CLUSTER VARPSU;
  WEIGHT PERWT&yy.F;
  DOMAIN education*employed;
run;
