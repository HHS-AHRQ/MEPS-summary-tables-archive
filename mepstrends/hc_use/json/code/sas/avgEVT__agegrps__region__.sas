...
/* Merge with FYC file */
  data FYCsub; set MEPS;
    keep agegrps region DUPERSID PERWT&yy.F VARSTR VARPSU;
  run;
...
proc means data = pers_events sum noprint;
  by DUPERSID VARSTR VARPSU PERWT&yy.F agegrps region ;
  var EXP;
...
proc surveymeans data = n_events mean missing nobs;
  FORMAT agegrps agegrps. region region.;
  VAR EXP;
  STRATA VARSTR;
  CLUSTER VARPSU;
  WEIGHT PERWT&yy.F;
  DOMAIN agegrps*region;
run;
