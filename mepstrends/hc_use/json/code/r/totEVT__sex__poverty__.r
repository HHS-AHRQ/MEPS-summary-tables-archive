...
# Keep only needed variables from FYC
  FYCsub <- FYC %>% select(sex,poverty,ind, DUPERSID, PERWT.yy.F, VARSTR, VARPSU)
...
results <- svyby(~(XP.yy.X >= 0), FUN=svytotal, by = ~sex + poverty, design = subset(EVNTdsgn, XP.yy.X >= 0))
print(results)
