...
# Keep only needed variables from FYC
  FYCsub <- FYC %>% select(sex,employed,ind, DUPERSID, PERWT.yy.F, VARSTR, VARPSU)
...
results <- svyby(~(XP.yy.X >= 0), FUN=svytotal, by = ~sex + employed, design = subset(EVNTdsgn, XP.yy.X >= 0))
print(results)
