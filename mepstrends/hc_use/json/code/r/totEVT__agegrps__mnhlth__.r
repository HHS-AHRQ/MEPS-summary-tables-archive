...
# Keep only needed variables from FYC
  FYCsub <- FYC %>% select(agegrps,mnhlth,ind, DUPERSID, PERWT.yy.F, VARSTR, VARPSU)
...
results <- svyby(~(XP.yy.X >= 0), FUN=svytotal, by = ~agegrps + mnhlth, design = subset(EVNTdsgn, XP.yy.X >= 0))
print(results)
