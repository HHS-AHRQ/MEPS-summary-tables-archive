...
# Keep only needed variables from FYC
  FYCsub <- FYC %>% select(married,employed,ind, DUPERSID, PERWT.yy.F, VARSTR, VARPSU)
...
results <- svyby(~(XP.yy.X >= 0), FUN=svytotal, by = ~married + employed, design = subset(EVNTdsgn, XP.yy.X >= 0))
print(results)
