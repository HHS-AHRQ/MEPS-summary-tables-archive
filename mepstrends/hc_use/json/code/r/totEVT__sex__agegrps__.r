...
# Keep only needed variables from FYC
  FYCsub <- FYC %>% select(sex,agegrps,ind, DUPERSID, PERWT.yy.F, VARSTR, VARPSU)
...
results <- svyby(~(XP.yy.X >= 0), FUN=svytotal, by = ~sex + agegrps, design = subset(EVNTdsgn, XP.yy.X >= 0))
print(results)
